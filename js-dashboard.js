...
// Authenticate Users card is only ever shown to these roles - the
...
// else. ADMIN included here since it inherits everything CHIEF JUROR/
// VICE CHIEF JUROR/MAP already have, on top of its own exclusive
// powers (see isSuperAdmin below - not built yet, Phase 2/3).
const canManageUsers = ['CHIEF JUROR', 'VICE CHIEF JUROR', 'MAP', 'ADMIN'].indexOf(userRole) !== -1;
if (canManageUsers) {
  document.getElementById('menu-authenticate-users').classList.remove('is-hidden');
}

// Groundwork for Phase 2/3: ADMIN-exclusive capabilities (see all
// submitted ratings, delete user, promote/demote CHIEF JUROR/VICE CHIEF
// JUROR/MAP, export forms) will check this flag. Unused for now.
const isSuperAdmin = userRole === 'ADMIN';
if (isSuperAdmin) {
  document.getElementById('menu-export-forms').classList.remove('is-hidden');
}

// Mirrors the backend's CHANGEABLE_ROLES list - only ADMIN ever sees
// this, via renderUserRow's role-change control below.
const CHANGEABLE_ROLES = ['JUROR', 'VICE CHIEF JUROR', 'CHIEF JUROR', 'MAP', 'ADMIN'];
...
const allViews = [menuView, organsView, submissionsView, detailView, usersView, userSessionsView, userSessionDetailView, questionsView, exportView];
...
function renderUsersList(users) {
  ...
  usersList.querySelectorAll('.user-setrole-btn').forEach(btn => {
    btn.addEventListener('click', () => handleSetUserRole(btn));
  });
...
function renderUserRow(u) {
  ...
  // ADMIN-exclusive: promote/demote this user's Role, or delete their
  // account outright. Hidden entirely for anyone else, and for the
  // logged-in ADMIN's own row (the backend blocks both self-role-change
  // and self-delete regardless, but there's no reason to show controls
  // that would just 400).
  const adminControlsHtml = (isSuperAdmin && u.username !== userSession.username) ? `
    <div class="user-decision-row">
      <select class="user-role-select">
        ${CHANGEABLE_ROLES.map(r => `<option value="${escapeHtml(r)}" ${r === u.role ? 'selected' : ''}>${escapeHtml(r)}</option>`).join('')}
      </select>
      <button type="button" class="btn-user-approve user-setrole-btn">Set Role</button>
      <button type="button" class="btn-user-reject user-delete-btn">Delete User</button>
    </div>
  ` : '';

  return `
    <div class="submission-item user-row" data-username="${escapeHtml(u.username)}" data-fullname="${escapeHtml(u.fullName)}" data-status="${escapeHtml(u.status)}" data-rowindex="${escapeHtml(u.rowIndex)}" data-role="${escapeHtml(u.role)}" style="cursor:default;">
...
// ADMIN-only: promotes/demotes a user's role. A confirmation is only
// shown when the change actually touches admin-tier access (promoting
// into or demoting out of CHIEF JUROR/VICE CHIEF JUROR/MAP/ADMIN) -
// picking the role someone already has, or changing between two
// equally-non-privileged states, doesn't need one.
async function handleSetUserRole(btn) {
  const row = btn.closest('.user-row');
  const fullName = row.dataset.fullname;
  const username = row.dataset.username;
  const rowIndex = Number(row.dataset.rowindex);
  ...
  const isPrivileged = r => ['CHIEF JUROR', 'VICE CHIEF JUROR', 'MAP', 'ADMIN'].indexOf(r) !== -1;
  const grantingAccess = isPrivileged(newRole) && !isPrivileged(currentRole);
  const revokingAccess = !isPrivileged(newRole) && isPrivileged(currentRole);

  if (grantingAccess) {
    if (!confirm(`Set ${fullName || username}'s role to ${newRole}? This grants them admin-tier access.`)) return;
  } else if (revokingAccess) {
    // At least as significant as granting it, arguably more so - this
    // was previously the one case that skipped confirmation entirely.
    if (!confirm(`Remove ${fullName || username}'s admin-tier access? They are currently ${currentRole} and would become ${newRole}.`)) return;
  } else if (isPrivileged(newRole) && isPrivileged(currentRole)) {
    if (!confirm(`Change ${fullName || username}'s role from ${currentRole} to ${newRole}?`)) return;
  }
  ...
  try {
    await callBackend('updateUserRole', { rowIndex, newRole });
    loadUsers();
  } catch (err) {
...
// ADMIN-only: permanently deletes a user's account. Always confirmed -
// this can't be undone, and unlike Revoke it removes the row entirely
...
async function handleDeleteUser(btn) {
  ...
  if (!confirm(`Permanently delete ${fullName || username}'s account? This cannot be undone. Their past submissions and ratings stay in the records, but they will need to sign up again to use the system.`)) {
    return;
  }
  ...
  try {
    await callBackend('deleteUser', { rowIndex });
    loadUsers();
  } catch (err) {
    alert(`Failed to delete this user: ${err.message}`);
    row.querySelectorAll('button, select').forEach(el => el.disabled = false);
    btn.textContent = originalLabel;
  }
}
...
// -----------------------------------------------------------
// EXPORT FORMS (ADMIN-only)
// -----------------------------------------------------------
const exportOrganSelect = document.getElementById('export-organ-select');
const exportOrganBtn    = document.getElementById('export-organ-btn');
const exportOrganHint   = document.getElementById('export-organ-hint');
const exportRatingsBtn  = document.getElementById('export-ratings-btn');
const exportRatingsHint = document.getElementById('export-ratings-hint');

let exportOrganOptionsLoaded = false;

async function ensureExportOrganOptions() {
  if (exportOrganOptionsLoaded) return;
  try {
    const organNames = await loadOrganNamesForAssignment();
    exportOrganSelect.innerHTML = `<option value="" selected disabled>Please select an organ</option>` +
      organNames.map(name => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('');
    exportOrganOptionsLoaded = true;
  } catch (err) {
    exportOrganHint.textContent = `Error loading organs: ${err.message}`;
    exportOrganHint.className = 'field-hint error';
  }
}

exportOrganSelect.addEventListener('change', () => {
  exportOrganBtn.disabled = !exportOrganSelect.value;
});

// Turns a CSV string into an actual browser download - same client-side
// Blob pattern as everything else in this app that produces a
// downloadable file, just plain text instead of a canvas/PDF.
...
exportOrganBtn.addEventListener('click', async () => {
  const organName = exportOrganSelect.value;
  if (!organName) return;

  const originalText = exportOrganBtn.textContent;
  exportOrganBtn.disabled = true;
  exportOrganBtn.textContent = 'Exporting…';
  exportOrganHint.textContent = '';
  exportOrganHint.className = 'field-hint';

  try {
    const result = await callBackend('exportOrganSubmissions', { organName });
    if (result.empty) {
      exportOrganHint.textContent = `${organName} has no submissions yet.`;
      exportOrganHint.className = 'field-hint';
    } else {
      downloadCsvString(result.csv, result.filename);
      exportOrganHint.textContent = 'Downloaded.';
      exportOrganHint.className = 'field-hint ok';
    }
  } catch (err) {
    exportOrganHint.textContent = err.message;
    exportOrganHint.className = 'field-hint error';
  } finally {
    exportOrganBtn.disabled = false;
    exportOrganBtn.textContent = originalText;
  }
});

exportRatingsBtn.addEventListener('click', async () => {
  const originalText = exportRatingsBtn.textContent;
  exportRatingsBtn.disabled = true;
  exportRatingsBtn.textContent = 'Exporting…';
  exportRatingsHint.textContent = '';
  exportRatingsHint.className = 'field-hint';

  try {
    const result = await callBackend('exportAllRatings', {});
    if (result.empty) {
      exportRatingsHint.textContent = 'No ratings have been submitted yet.';
      exportRatingsHint.className = 'field-hint';
    } else {
      downloadCsvString(result.csv, result.filename);
      exportRatingsHint.textContent = 'Downloaded.';
      exportRatingsHint.className = 'field-hint ok';
    }
  } catch (err) {
    exportRatingsHint.textContent = err.message;
    exportRatingsHint.className = 'field-hint error';
  } finally {
    exportRatingsBtn.disabled = false;
    exportRatingsBtn.textContent = originalText;
  }
});

document.getElementById('back-to-menu-from-export').addEventListener('click', () => showView(menuView));

document.getElementById('menu-export-forms').addEventListener('click', () => {
  showView(exportView);
  ensureExportOrganOptions();
});
